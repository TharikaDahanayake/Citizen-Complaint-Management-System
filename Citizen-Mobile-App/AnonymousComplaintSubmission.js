...
export default function AnonymousComplaintSubmission({ citizen, onBack }) {
  ...
  const validateForm = () => {
    ...
    if (latitude === null || longitude === null) {
      return 'Please select a location to capture latitude and longitude.';
    }
  ...
  const uploadEvidenceFiles = async () => {
    ...
    const uploads = evidenceAssets.map(async (asset, index) => {
      try {
        ...
        const fileName = `anonymous_complaint_${Date.now()}_${index}.${safeExtension}`;
        const contentType = asset.mimeType || (safeExtension === 'mp4' ? 'video/mp4' : 'image/jpeg');

        const formData = new FormData();
        formData.append('upload_preset', CLOUDINARY_UPLOAD_PRESET);
        formData.append('folder', 'complaints/anonymous');
  ...
  return (
    <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
      <Text style={styles.heading}>Anonymous Complaint Submission</Text>
      <Text style={styles.subtitle}>Submit a complaint without sharing your personal identity.</Text>
      ...
      <TouchableOpacity style={styles.dateFieldButton} onPress={openDatePicker} activeOpacity={0.8}>
